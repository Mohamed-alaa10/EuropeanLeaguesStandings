...
class App extends Component {
    ...
    fetchData(id, name) {
        const Token = '8993f2b1a02e4186acf0cec9b7e71228';
        const URL = 'https://api.football-data.org/v4/competitions/' + id + '/standings';
                ...
                <div className="container">
                    <div className="row">
                        ...
                        {this.state.leagues.map(league => (
                            <button className="btn btn-light mr-2 mt-2" key={league.id} onClick={() => {this.handleSelection(league.id, league.name)}}>{league.name}</button>
                        ))}
                        </div>
                    </div>
